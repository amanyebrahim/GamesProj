package com.roqay.saheby.utils

import android.content.Context
import android.graphics.*
import android.net.Uri
import com.example.gamesproj.utils.extension.getBitmapFromUri
import java.io.File
...
fun calculateSampleSize(
    ...
    return when (scalingLogic) {
        ScalingLogic.FIT -> {
            ...
            when {
                srcAspect > dstAspect -> srcWidth / dstWidth
                else -> srcHeight / dstHeight
            }
        }
            ...
            when {
                srcAspect > dstAspect -> srcHeight / dstHeight
                else -> srcWidth / dstWidth
            }
        }
    }
}
...
fun calculateSrcRect(
    ...
    return when (scalingLogic) {
        ...
        else -> Rect(0, 0, srcWidth, srcHeight)
    }
}
...
fun calculateDstRect(
    ...
    return when (scalingLogic) {
        ScalingLogic.FIT -> {
            ...
            when {
                srcAspect > dstAspect -> Rect(0, 0, dstWidth, (dstWidth / srcAspect).toInt())
                else -> Rect(0, 0, (dstHeight * srcAspect).toInt(), dstHeight)
            }
        }

        else -> Rect(0, 0, dstWidth, dstHeight)
    }
}
